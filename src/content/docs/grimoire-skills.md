...
**When to use:** When two or more installed skills give contradictory guidance for the same situation, or to proactively scan preferences and rank conflicts before one arises.

**What it produces:** A recorded precedence decision — which skill wins — saved so the same conflict never has to be re-resolved.

**Example invocation:**

```
User: "design-api-versioning-strategy says use URL versioning, but our pinned REST skill says header versioning. Which wins?"
→ resolve-best-practice-conflict: surfaces both positions, asks which the user prefers,
  saves the decision as a pinned preference for this subdomain
```
...
**When to use:** When the user wants to install or uninstall skills by domain or individual skill, upgrade grimoire, clean up broken symlinks, list available skills, or health-check the installation.

**What it produces:** A guided interface over the `grimoire` CLI — shows the command, confirms before destructive operations, reports exactly what changed.

**Example invocation:**

```
User: "Uninstall the cooking domain, I don't need it."
→ install-grimoire: confirms target, runs the uninstall, reports which paths were removed
```
...
```
User: "Apply BPDD to this repo using our pinned engineering profile."
→ apply-best-practice-driven-development: checks compliance, fixes findings one at a time,
  re-checks until the artifact matches the declared standard
```
...
**When to use:** When the user wants to check whether an artifact aligns with their pinned preferences — "check compliance", "linter for best practices," "are we following our pinned practices?"

**What it produces:** A compliance report against the resolved effective preferences — LSP-compatible JSON plus an HTML coverage report. The "linter for best practices."

**Example invocation:**

```
User: "Check this PR against our pinned standards before I merge."
→ check-best-practice-compliance: runs the resolved preference set against the diff,
  reports pass/fail per rule with file:line locations
```
...
**When to use:** When the user wants to fix one specific finding from a `check-best-practice-compliance` report — "fix this finding", "resolve violation #3."

**What it produces:** A targeted, location-aware fix for exactly the cited finding — no unrelated changes.

**Example invocation:**

```
User: "Fix finding #3 — the SRP violation in UserService."
→ fix-best-practice-finding: loads the finding's location, applies the minimal fix,
  re-runs the specific check to confirm it now passes
```
...
**When to use:** When the user wants to activate a named set of best practices for a paradigm or methodology — "use OOP", "set profile to clean-architecture", "switch to functional style."

**What it produces:** The named profile's skills pinned as preferences for the current scope (session, project, or global).

**Example invocation:**

```
User: "Switch this project to the clean-architecture profile."
→ apply-best-practice-profile: resolves the profile, pins its skill set as project preferences
```
...
**When to use:** When the user wants to create a new practice profile — "create a profile for my team", "write an OOP profile."

**What it produces:** A `.grimoire/profiles/<name>.toml` file bundling a curated skill set for a paradigm, team, or project context.

**Example invocation:**

```
User: "Make a backend-defaults profile from our current pinned preferences."
→ write-best-practice-profile: scaffolds the profile file, populates it from current pins
```
...
**When to use:** When the user wants to validate a profile before using or sharing it — "review my profile", "validate my-team.toml."

**What it produces:** A validation pass — format check, verifies every referenced skill exists, surfaces internal conflicts — before the profile is trusted or shared.

**Example invocation:**

```
User: "Validate my-team.toml before I share it."
→ review-best-practice-profile: checks format and skill references, flags 1 conflicting entry
```
...
**When to use:** When the user wants to publish or share a practice profile — "share my team profile", "open-source my backend-defaults profile."

**What it produces:** A published, installable profile others can adopt directly, following the same "one shareable file replaces manual selection" pattern as `eslint-config-*` packages.

**Example invocation:**

```
User: "Publish my backend-defaults profile so the rest of the team can use it."
→ share-best-practice-profile: runs review-best-practice-profile first, then publishes
```
